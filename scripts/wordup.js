...
function checkIfWordIsReal(word) {
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            for (var a in model.wordSubmissions) {
                if (model.wordSubmissions[a].word == word){
                    (model.wordSubmissions[a])['isRealWord'] = theAnswer;
                }
            }
...
function render() {
    ...
    if (disallowedLetters.length > 0) {
        ...
        $("#wrongletter").append(redLetterChips);
...
function wordSubmissionChip(wordSubmission) {
    ...
    if (wordSubmission.hasOwnProperty("isRealWord")) {
        var scoreChip = $("<span></span>").text(wordScore(wordSubmission.word)).addClass('tag  tag-sm tag-primary').attr('id', 'scoreChip');
        // TODO 17(done)
        // give the scoreChip appropriate text content
        if (wordSubmission.isRealWord == false){
            scoreChip = $("<span></span>").text("X").addClass("tag tag-sm tag-danger").attr('id', 'scoreChip')
        }

        // TODO 18(done)
        // give the scoreChip appropriate css classes
        $('scoreChip').addClass('bad-attempt');
        // TODO 16(done)
...
$(document).ready(function() {
    ...
    $('#textbox').on("input", function() {
        model.currentAttempt = document.getElementById('textbox').value;
        render();
    });
